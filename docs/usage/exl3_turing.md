...
export VLLM_EXL3_NGRAM_STREAM=1
...
Both streaming variables are required for Qwen Flash PLE packs.
`VLLM_EXL3_NGRAM_STREAM=1` enables the vLLM filtering loader, while
`EXL3_NGRAM_STREAM=1` keeps the n-gram embedding table in the runtime's SSD
streaming mode. Non-filtering checkpoint loaders are deliberately rejected by
the vLLM loader integration.
