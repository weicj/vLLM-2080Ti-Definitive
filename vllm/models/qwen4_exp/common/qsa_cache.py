...
class QSAStateBackend(AttentionBackend):
    """Key-only backend for out-of-band QSA side-cache operations."""
    ...
    def indexes_kv_by_block_stride(cls) -> bool:
        # QSA pages use a blocks-first physical view, so the allocator may
        # stride over the padding inserted between pages when hybrid KV
        # owners have different per-page sizes.
        return True
    ...
    def get_kv_cache_shape(
        ...
        # QSA kernels consume [pages, page_size, 1, width].
...
class QSAKeyStateCache(_QSAStateCache):
    ...
    def bind_kv_cache(self, kv_cache: torch.Tensor) -> None:
        ...
        if kv_cache.dtype != self.dtype or kv_cache.shape[3] != self.head_size:
            raise ValueError("QSA raw cache does not match its cache spec")
