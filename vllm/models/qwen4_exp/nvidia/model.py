...
class Qwen4ExpModel(nn.Module):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        if self.hyper_connection_mixer is None:
            # The final HC mixer is instantiated only on the last PP stage.
            # Checkpoint weights are still presented to every stage, so remove
            # this stage-local subtree before AutoWeightsLoader validates names.
            weights = (
                (name, weight)
                for name, weight in weights
                if "hyper_connection_mixer." not in name
            )
        weights = (
            (
