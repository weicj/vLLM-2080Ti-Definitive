...
class Qwen4ExpQSAFlashAttentionImpl(FlashAttentionImpl):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if self.kv_cache_dtype not in ("auto", "float16", "bfloat16"):
            raise NotImplementedError("Qwen4Exp QSA requires an FP16 or BF16 main KV cache")
    ...
    def forward_qsa(
        ...
        if key_cache.dtype not in supported_dtypes or query.dtype != key_cache.dtype:
            raise NotImplementedError("Qwen4Exp QSA requires matching FP16 or BF16 Q/K/V")
...
class Qwen4ExpQSAAttention(Qwen3NextAttention, AttentionLayerBase):
    ...
    def __init__(
        ...
        *,
        ...
        if self.kv_cache_torch_dtype not in (torch.float16, torch.bfloat16):
            raise NotImplementedError("Qwen4Exp QSA requires FP16 or BF16 cache storage")
