...
from ..common.ple import DiskMappedPLEEmbedding, copy_ple_embedding_shard_
...
class Qwen4ExpNGramEmbedding(PleOffloadLayer):
    def __init__(
        ...
        if envs.VLLM_PLE_CPU_OFFLOAD and is_offload_process():
            ...
            self.ngram_embedding = DiskMappedPLEEmbedding(
                ...
                key_prefix=prefix,
                org_vocab_size=padded_vocab_size,
    ...
    def forward_impl(  # type: ignore[override]
        ...
        if getattr(self.ngram_embedding, "is_disk_mapped", False):
            if output_buffer is None:
                output_buffer = torch.empty(
                    num_tokens,
                    self.embedding_dim,
                    dtype=self.ngram_embedding.weight_dtype,
                )
            # The offload worker may reuse a graph-sized output workspace.  A
            # lookup must receive only the rows represented by this request;
            # passing the full workspace makes the CPU gather shape exceed the
            # actual ngram-id count for small requests.
            output = output_buffer[:num_tokens, : self.embedding_dim]
            flat_output = output.reshape(-1, self.head_dim)
            self.ngram_embedding.lookup(ngram_ids.reshape(-1), out=flat_output)
            return output
    ...
    def get_offload_output_dtype(self, default_dtype: torch.dtype) -> torch.dtype:
        ...
        if weight is not None:
            return weight.dtype
        mapped_dtype = getattr(embedding, "weight_dtype", None)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            ...
            if name.startswith(shard_prefix) and name.endswith(".weight"):
                if getattr(self.ngram_embedding, "is_disk_mapped", False):
                    # The CPU offload process reads these rows from the
                    # safetensors mapping on demand; never materialize them.
                    continue
